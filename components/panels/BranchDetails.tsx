            ...
            <div>
                <div className="flex items-center justify-between mb-2">
                    <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 flex items-center gap-2">
                        Tasks
                        {sortedTasks.length > 0 && <span className="px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-[10px]">{sortedTasks.length}</span>}
                    </label>
                    <div className="flex items-center gap-3">
                        {sortedTasks.length > 0 && (
                            <button 
                                ...
                                className={`text-xs flex items-center gap-1 transition-colors ${isBulkMoveMode ? 'text-indigo-600 font-bold' : 'text-slate-500 hover:text-indigo-600'}`}
                            >
                                <Move className="w-3 h-3" />
                                {isBulkMoveMode ? 'Annulla Sposta' : 'Sposta Bulk'}
                            </button>
                        )}
                        <button 
                    ...
                    <div className="space-y-3">
                        ...
                        {isBulkMoveMode && selectedTaskIds.length > 0 && (
                            <div className="p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-lg animate-in slide-in-from-top-2">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-xs font-bold text-indigo-700 dark:text-indigo-300">{selectedTaskIds.length} task selezionati</span>
                                    <button onClick={() => setSelectedTaskIds([])} className="text-[10px] text-indigo-600 hover:underline">Deseleziona tutto</button>
                                </div>
                                <div className="flex gap-2">
                                    <select
                                        value={bulkMoveTargetId}
                                        onChange={(e) => setBulkMoveTargetId(e.target.value)}
                                        className="flex-1 text-xs bg-white dark:bg-slate-800 border border-indigo-200 dark:border-indigo-800 rounded-md p-2 outline-none focus:ring-2 focus:ring-indigo-500"
                                    >
                                        <option value="">Sposta in...</option>
                                        {(Object.values(state.branches) as Branch[]).filter(b => b.id !== branch.id).map(b => (
                                    ...
                                    <button 
                                        ...
                                        className="px-3 py-1 bg-indigo-600 text-white rounded-md text-xs font-bold disabled:opacity-50 transition-colors"
                                    >
                                        Vai
                                    </button>
                                </div>
                        ...
                        <ul className="space-y-2">
                            {sortedTasks.length === 0 && <p className="text-sm text-gray-400 italic text-center py-4">Nessun task.</p>}
                            {sortedTasks.map((task, index) => (
                                <li key={task.id} className={`group bg-white dark:bg-slate-800 border rounded-md p-2 hover:shadow-sm transition-all relative ${selectedTaskIds.includes(task.id) ? 'border-indigo-400 ring-1 ring-indigo-400/20 bg-indigo-50/30' : 'border-gray-100 dark:border-slate-700'}`}>
                                    <div className="flex items-start gap-2">
                                        {isBulkMoveMode ? (
                                            <button 
                                                onClick={() => toggleTaskSelection(task.id)}
                                                className={`mt-0.5 ${selectedTaskIds.includes(task.id) ? 'text-indigo-600' : 'text-slate-300 hover:text-indigo-400'}`}
                                            >
                                                {selectedTaskIds.includes(task.id) ? <CheckSquare className="w-5 h-5" /> : <Square className="w-5 h-5" />}
                                            </button>
                                        ) : (
                                            <button 
                                        ...
                                        <div className="flex-1 min-w-0 space-y-2">
                                            <div className="flex items-center gap-2">
                                                <p 
                                                    ...
                                                    onClick={() => {
                                                        if (isBulkMoveMode) toggleTaskSelection(task.id);
                                                        else setEditingTask({ branchId: branch.id, taskId: task.id });
                                                    }}
