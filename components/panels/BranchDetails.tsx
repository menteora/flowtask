...
const BranchDetails: React.FC = () => {
  ...
  // Stati per il buffering (salvataggio esplicito)
  ...
  // Sincronizza lo stato locale quando cambia il ramo selezionato
  useEffect(() => {
    if (branch) {
  ...
  const inheritedResp = localResponsibleId ? undefined : getInheritedResponsible(branch.id);
  const currentResp = localResponsibleId ? state.people.find(p => p.id === localResponsibleId) : undefined;
  ...
  const handleBulkSave = () => {
    ...
    setIsBulkMode(false);
    showNotification("Task aggiornati con successo.", "success");
  };
  ...
  const handleBranchMoveToProject = async () => {
      ...
      if (!confirm(`Sei sicuro di voler spostare il ramo "${localTitle}" e TUTTA la sua gerarchia nel progetto selezionato?`)) return;
      try {
  ...
  return (
    <div className="fixed inset-0 z-50 md:absolute md:inset-auto md:right-0 md:top-0 md:bottom-0 md:w-96 bg-white dark:bg-slate-900 md:border-l border-gray-200 dark:border-slate-700 flex flex-col shadow-xl">
      <div className="p-4 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center bg-gray-50 dark:bg-slate-800/50">
        <div className="flex-1 mr-4">
           <span className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2">
               {localIsLabel ? 'Etichetta' : (localIsSprint ? 'Sprint Mode' : 'Dettagli Ramo')}
           ...
           <div className="flex items-center gap-2 mt-1">
             <input 
                ...
                className={`font-bold text-lg bg-transparent border-b outline-none transition-all w-full ${localTitle !== branch.title ? 'border-amber-400 text-amber-700 dark:text-amber-400' : 'border-transparent text-slate-800 dark:text-white'}`} 
             />
           </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 relative custom-scrollbar">
        ...
        {/* Barra di salvataggio Dirty */}
        {isDirty && (
            <div className="sticky top-0 z-40 flex items-center gap-2 p-3 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-xl shadow-md animate-in slide-in-from-top-4">
                <div className="flex-1">
                    <p className="text-[10px] font-black text-amber-700 dark:text-amber-400 uppercase tracking-widest">Modifiche non salvate</p>
                </div>
                <button 
                    onClick={handleReset}
                    className="p-2 text-amber-600 hover:bg-amber-100 dark:hover:bg-amber-800 rounded-lg transition-colors"
                ...
                <button 
                    ...
                    className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold shadow-sm flex items-center gap-2"
                >
            ...
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-sm">
                ...
                <div className="p-3 border-b border-slate-100 dark:border-slate-700">
                    <div className="flex items-center justify-between mb-2">
                        ...
                        {inheritedResp && !localResponsibleId && (
                    ...
                    <div className="flex items-center gap-3">
                        <div className="relative flex-1">
                            <div className={`flex items-center gap-2 p-2 rounded-lg border transition-all ${localResponsibleId ? (localResponsibleId !== branch.responsibleId ? 'border-amber-400 bg-amber-50/20' : 'border-indigo-300 bg-indigo-50/30 dark:border-indigo-800 dark:bg-indigo-900/10') : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900'}`}>
                                {currentResp ? (
                                ...
                                <span className={`text-xs truncate ${!localResponsibleId && inheritedResp ? 'text-slate-400 italic' : 'text-slate-700 dark:text-slate-200 font-medium'}`}>
                                    {localResponsibleId ? currentResp?.name : (inheritedResp ? inheritedResp.name : 'Nessuno')}
                                </span>
                                <select 
                                    value={localResponsibleId || ''} 
                                    onChange={(e) => setLocalResponsibleId(e.target.value || undefined)}
                        ...
                        {localResponsibleId && (
                            <button 
                                onClick={() => setLocalResponsibleId(undefined)}
                ...
                <div className="p-2 grid grid-cols-3 gap-1 bg-white dark:bg-slate-900">
                    {[
                        { id: 'normal', icon: GitBranch, label: 'Normale', active: !localIsLabel && !localIsSprint, onClick: () => { setLocalIsLabel(false); setLocalIsSprint(false); } },
                        { id: 'label', icon: Tag, label: 'Label', active: localIsLabel, onClick: () => { setLocalIsLabel(true); setLocalIsSprint(false); } },
                        { id: 'sprint', icon: Zap, label: 'Sprint', active: localIsSprint, onClick: () => { setLocalIsLabel(false); setLocalIsSprint(true); } }
                    ].map(type => (
                        <button 
                            key={type.id}
                    ...
                    <div className="p-4 bg-indigo-50/30 dark:bg-indigo-900/10 border-t border-indigo-100 dark:border-indigo-900/30 animate-in slide-in-from-top-2">
                        <div className="flex items-center justify-between gap-4">
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-[9px] text-slate-500 italic truncate">Esempio: {localTitle} {new Date().getFullYear().toString().slice(-2)}-{String(localSprintCounter || 1).padStart(2, '0')}</p>
                            </div>
                            <div className="shrink-0 flex items-center gap-2 bg-white dark:bg-slate-800 p-1.5 rounded-lg border border-indigo-200 dark:border-indigo-800">
                                ...
                                <input 
                                    type="number" 
                                    min="1" 
                                    value={localSprintCounter || 1} 
                                    onChange={(e) => setLocalSprintCounter(Math.max(1, parseInt(e.target.value) || 1))}
        ...
        <div className="space-y-2">
            <div className="flex items-center justify-between">
                 ...
                 <div className="flex gap-2">
                    {localDescription !== (branch.description || '') && (
                        <span className="text-[9px] text-amber-500 font-black uppercase">Modificata</span>
                    )}
            ...
            {isPreviewMode ? (
                ...
                <div className={`border rounded-lg overflow-hidden relative focus-within:ring-2 focus-within:ring-indigo-500/50 transition-all ${localDescription !== (branch.description || '') ? 'border-amber-400' : 'border-slate-200 dark:border-slate-700'}`}>
                    ...
                    <textarea ref={textareaRef} value={localDescription} onChange={(e) => setLocalDescription(e.target.value)} className="w-full h-28 p-3 text-sm bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 focus:outline-none resize-y" placeholder="Descrizione..." />
                </div>
            )}
        </div>
            ...
            <div>
                ...
                {isBulkMode ? (
                    <div className="space-y-2">
                        ...
                        <button onClick={handleBulkSave} className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl flex items-center justify-center gap-2 text-sm font-black uppercase shadow-lg"><Save className="w-4 h-4" /> Salva Tutti i Task</button>
                    </div>
                ) : (
        ...
        <div className="space-y-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-700">
            ...
            <div>
                ...
                <DatePicker 
                    ...
                    className={`w-full bg-white dark:bg-slate-800 border rounded p-2 min-h-[36px] ${localStartDate !== branch.startDate ? 'border-amber-400' : 'border-gray-200 dark:border-slate-600'}`}
                />
            </div>
            <div>
                 ...
                 <DatePicker 
                    value={localDueDate}
                    onChange={(val) => setLocalDueDate(val)}
                    placeholder="Fine progetto"
                    className={`w-full bg-white dark:bg-slate-800 border rounded p-2 min-h-[36px] ${localDueDate !== branch.dueDate ? 'border-amber-400' : 'border-gray-200 dark:border-slate-600'}`}
                />
            </div>
        </div>
                ...
                {Object.keys(BranchStatus).map((statusKey) => {
                  ...
                  return (
                    <button 
                        ...
                        className={`flex items-center justify-center gap-2 px-3 py-2 rounded-md text-sm font-medium border transition-all ${isActive ? `${config.color} border-current ring-1 ring-current` : 'border-gray-200 dark:border-slate-700 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}
                    >
